...
sub import {
    ...
    $Config{dir} ||= File::Spec->catdir($CPAN::Config->{cpan_home},"plugins",__PACKAGE__);
}
...
EOF
    ...
    File::Path::mkpath($Config{dir});
    open my $specout, ">", "$Config{dir}/perl-$dist.spec" or die;
    print $specout $ret;
    $CPAN::Frontend->myprint("Wrote $Config{dir}/perl-$dist.spec");
    $ret;
}
