...
=item * CPAN + CPAN::Reporter + 1 Prise Konfiguration
...
In the works: C<cc_skipfile>, C<send_skipfile>
...
=item * weil es eine ganze Menge Spaß macht
...
zu existenziellen Katastrophen.

=head1 Existentielle Katastrophen
...
auszusetzen. Schließlich tun sie das alles aus eigenen Stuecken, also
haben wir auch nichts zu fordern. Wenn ein Tester kein Interesse an
unseren Diensten hat, hands down.

Ich bemühe mich dann so gut ich kann, einen Patch zu schreiben, eine
Distroprefs-Datei zu schreiben, nur eine kurze Notiz an den Autor.
Oder ganz ab in den Killfile.
...
Es gibt Tests, die laufen zu lange.

=item * Hänger

Es gibt Tests, die bleiben einfach stehen.
...
=item * http://log.perl.org

Announcement pending on cpan-uploads@perl.org, but alternatives
